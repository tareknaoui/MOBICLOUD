...
class SendMemberUpdateUseCase @Inject constructor(
    ...
    suspend operator fun invoke(update: MemberUpdate) {
        ...
        // H8 : pour LEFT, on INCLUT le leaver dans le broadcast. Le membre évincé reçoit
        // ainsi son propre LEFT et peut déclencher un re-JOIN immédiat plutôt que d'attendre
        // SP_TIMEOUT (90 s) et de continuer à envoyer des HB inutiles dans l'intervalle.
        // Soi-même (le SP émetteur) reste exclu : pas de boucle de message.
        val memberNodeIds = members
