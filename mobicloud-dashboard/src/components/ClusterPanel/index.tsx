    ...
    <div style={{
      ...
      >
        ...
        <span style={{ fontSize: 10, color: 'var(--accent-blue)', fontFamily: 'monospace', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {cluster.clusterId === '__no_cluster__' ? '(no cluster)' : cluster.clusterId.slice(0, 14) + '…'}
        </span>
      ...
      {!expanded && (
        <div style={{ padding: '0 10px 7px', display: 'flex', flexDirection: 'column', gap: 3 }}>
          {cluster.superPeer ? (
            ...
            <div style={{ fontSize: 10, color: 'var(--accent-red)' }}>⚠ No Super-Peer</div>
          )}
          <div style={{ display: 'flex', gap: 10, fontSize: 10, color: 'var(--text-sec)' }}>
            <span>Free: <strong style={{ color: 'var(--text-primary)' }}>{fmtBytes(cluster.totalFreeBytes)}</strong></span>
            <span>Avg rel.: <strong style={{ color: reliabilityColor(cluster.avgReliabilityScore) }}>{(cluster.avgReliabilityScore * 100).toFixed(0)}%</strong></span>
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', background: 'var(--bg-card)', borderRadius: 10, border: '1px solid var(--border)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '6px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 9, color: 'var(--text-dim)', fontStyle: 'italic' }}>click for storage details</span>
      ...
      <div style={{ overflowY: 'auto', padding: '6px 8px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {(!data || data.clusters.length === 0) && (
          <div style={{ color: 'var(--text-muted)', fontSize: 11, fontStyle: 'italic', padding: 8 }}>No active cluster</div>
        )}
        {data?.clusters.map(c => (
          <ClusterCard
            key={c.clusterId}
